...
- [RFC: server MCP locale](RFC_SERVER_MCP_LOCALE.md) — bozza (§15 Slice B): server stdio per-sessione che espone bacheca/piano/note/registro come tool tipizzati chiamando le funzioni di dominio (mai subprocess CLI), così gli agenti rispondono senza sillabare comandi di shell. Non riporta Gemini nel dispatch headless (risponde a tool call, non avvia turni). MVP: letture + `bacheca_rispondi`/`bacheca_prendi`/`piano_prendi_passo`. Esclusi: I/O generico, dispatch, toggle Postino, git.
